...
class SampleErvyCommand extends Command {
  async run() {
    const { flags } = this.parse(SampleErvyCommand);
    const name = flags.name || "world";
    this.log(
      `hello ${name} from C:\\Users\\v__V_\\_V_\\NODEjs_TryOuts\\-v-\\src\\commands\\sample_ervy.js`
    );

    const ervy = require("ervy");
    const { bar, pie, bullet, donut, gauge, scatter, bg, fg } = ervy;
    ...
    for (let i = 1; i < 17; i++) {
      i < 6
        ? scatterData.push({ key: "A", value: [i, i], style: fg("red", "*") })
...
module.exports = SampleErvyCommand;
